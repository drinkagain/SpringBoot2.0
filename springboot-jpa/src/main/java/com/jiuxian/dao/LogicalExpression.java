...
public class LogicalExpression<T> implements Specification<T> {
    ...
    public LogicalExpression(QueryCriteria[] querySpecification, Operator operator) {
        this.QueryCriteria = querySpecification;
    ...
    public Predicate toPredicate(Root<T> root, CriteriaQuery<?> query, CriteriaBuilder builder) {
        ...
        if (QueryCriteria != null) {
            for (int i = 0; i < QueryCriteria.length; i++) {
                predicates.add(QueryCriteria[i].toPredicate(root, query, builder));
            }
        }
        switch (operator) {
            case OR:
